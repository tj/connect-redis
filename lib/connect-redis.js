...
function getSlaves () {
  ...
  self.master.send_command('ROLE', [], function (err, role) {
    ...
    if (role.role === 'master') {
      role.slaves.forEach(function (slave) {
        slave.name = slave.host + ':' + slave.port;
        if (!self.slaves[slave.name]) {

          self.slaves[slave.name] = new redis.createClient(slave.port, slave.host, self.options);

          self.slaves[slave.name].name = slave.name;
          slave = self.slaves[slave.name];

          slave.on('error', function () {
            self.emit('slave-disconnect', slave.name);
          });
          slave.on('error', function () {
            self.emit('slave-connect', slave.name);
          });
